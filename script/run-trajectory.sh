...
mpirun -n 4 python plot_surface.py --mpi --cuda --model CifarResNetBasic --num-blocks ${NET} ${COOR_ARGS} --model_file ${MODEL_PATH}/model_pad_${MAX_EPOCH}.t7 --dir_type weights --xignore biasbn --yignore biasbn --proj_file ${MODEL_PATH}/PCA_weights_ignore\=biasbn_save_epoch\=${SAVE_EPOCH}/directions.h5_proj_cos.h5 --dir_file ${MODEL_PATH}/PCA_weights_ignore\=biasbn_save_epoch\=${SAVE_EPOCH}/directions.h5  --plot --data-parallel
